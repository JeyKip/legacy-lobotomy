...
class TestProfileManagement:
    ...
    def test_get_users_method_should_return_404_not_found_error(self, regular_user, auth_client_factory):
    ...
    def test_authenticated_user_cannot_see_details_of_any_other_user_in_the_system(
            self, regular_user, regular_user_factory, auth_client_factory
    ):
        another_user = regular_user_factory()
    ...
    @pytest.mark.django_db
    def test_authenticated_user_cannot_make_another_user_a_superuser(
            self, regular_user, regular_user_factory, auth_client_factory
    ):
        # first_login=False is used to work around a bug with attempt to change request data
        another_user = regular_user_factory(first_login=False)
    ...
    def test_patch_request_should_not_be_allowed(self, regular_user, auth_client_factory):
        auth_client = auth_client_factory(regular_user)
        response = auth_client.patch(f'/api/users/{regular_user.pk}/', {'first_name': 'John'})

        assert response.status_code == status.HTTP_404_NOT_FOUND

    @pytest.mark.django_db
    def test_authenticated_user_cannot_delete_another_user(
            self, regular_user, regular_user_factory, auth_client_factory
    ):
        another_user = regular_user_factory()
