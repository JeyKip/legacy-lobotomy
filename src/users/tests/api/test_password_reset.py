...
class TestPasswordReset:
    ...
    def test_when_user_with_specified_email_exists_then_password_reset_email_should_be_sent(
            self, make_token_mock, faker, uid, regular_user, client, mailoutbox, settings, secure, expected_schema, site
    ):
        ...
        response = client.post(self.reset_password_endpoint, {'email': regular_user.email}, secure=secure)
        ...
        assert email.to == [regular_user.email]
...
{expected_schema}://{site.domain}/password-reset-confirm/{uid(regular_user.pk)}/{password_reset_token}/

Your email, in case you've forgotten: {regular_user.email}
...
Please click on the following link and choose a new password:
<br>
<a href="{expected_schema}://{site.domain}/password-reset-confirm/{uid(regular_user.pk)}/{password_reset_token}/">{expected_schema}://{site.domain}/password-reset-confirm/{uid(regular_user.pk)}/{password_reset_token}/</a>
<br>
Your email, in case you’ve forgotten: {regular_user.email}
<br>
Thanks for using our site!
